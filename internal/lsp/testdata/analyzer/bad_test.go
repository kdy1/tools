...
func Testbad(t *testing.T) { //@diag("", "tests", "Testbad has malformed name: first letter after 'Test' must not be lowercase", "warning")
	...
	printfWrapper("%s") //@diag(re`printfWrapper\(.*\)`, "printf", "github.com/kdy1/tools/internal/lsp/analyzer.printfWrapper format %s reads arg #1, but call has 0 args", "warning")
}
